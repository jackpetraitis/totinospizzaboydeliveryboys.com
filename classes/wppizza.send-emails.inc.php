...
//			}
			...
			/**if name is still empty, but we have an email address, use that as name **/
			if(empty($orderFromName) && !empty($orderFromEmail)){
				$orderFromName = $orderFromEmail;
			}
		...
		**********************************/
			if($asHtml){
				...
				************************************************/
				...
				/***********************************************
					set the html body. use only html part here
					to be overridden by phpmailer_init action hook
				...
				************************************************/
				$message = $email_markup['html'];
				/***********************************************
					set plaintext and html markup we can  use 
					in phpmailer_init action hook
				************************************************/
				$this->message_body['html'] = $email_markup['html'];
				$this->message_body['plaintext'] = $email_markup['plaintext'];

				/*************************************************
					set headers
					Content-TYPE, HEADERS, FROM, CC, BCC, REPLY TO
				*************************************************/
				$headers = array();
				...
				/************************************************
					content type / charset				
				************************************************/
				$headers[] = 'Content-Type: text/html; charset='.WPPIZZA_CHARSET.'';
				...
				************************************************/
				if($orderFromEmail!=''){
					$headers[] = 'From: '.$orderFromName.' <'.$orderFromEmail.'>';
				}				
				...
				************************************************/
				if(!empty($email_reply_to)){
					...
					$headers[] = 'Reply-To: '.$rtName.' <'.$email_reply_to.'>';
				}				
				...
				*************************************************/
				...
				if(!empty($options['tools']['disable_emails'])){
					...
					if(wp_mail( $recipients, $subject, $message, $headers, $attachments )) {
						$send_email_results['status']=true;
					}else{
						$send_email_results['status']=false;
						$send_email_results['error']=$GLOBALS['phpmailer']->ErrorInfo;
	...
	********************************************************************************/	
	function set_html_mail_body($phpmailer){
		...
		if( $phpmailer->ContentType == 'text/html' ) {
			if(!empty($this->message_body['html'])){
				/** if MsgHTML is set, phpmailer will automatically set this as body */
	...
	/*********************************************************************************
	*
	*	[EMAILS : log errors - currently not in use as it's output anyway]
	*
	********************************************************************************/	
	//function log_mailer_errors($wp_error){	
  	//	$error_log_file = WPPIZZA_PATH . '/logs/mail-error-'.wp_hash('wppizzamail').'.log';
	//	file_put_contents($error_log_file,'['.date('Y-m-d').']: ' . print_r($wp_error->errors,true).''.PHP_EOL, FILE_APPEND);
	//}
	/*********************************************************************************
	*
